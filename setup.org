...
* key-bindings
  ...
  #+BEGIN_SRC emacs-lisp
    ...
    (define-key global-map (kbd "C-: e o") 'olivetti-mode)
    ...
    (define-key global-map (kbd "C-: o k") 'consult-yank-from-kill-ring)
  #+END_SRC
  ...
  #+BEGIN_SRC emacs-lisp
    ...
    (defun extra_customize ()
      ...
      (setq org-todo-keywords
            '((sequence "TODO" "FEEDBACK" "VERIFY" "INPROGRESS" "DEPENDENT" "|" "PAUSED" "DONE" "BLOCKED" "CANCELED")))
      ...
      ;; (set-face-attribute 'hl-line nil :inherit t :background "gray27")
      ...
      (use-package web-beautify
        :ensure t
        :config
        (defun verify-js-beautify-installation ()
          "check if node package js-beautify is installed
       if not then, try to install it"
          (defvar is-installed (shell-command "npm list -g js-beautify"))
          (defconst SUCCESS_CODE 0)
          (if (not (eq is-installed SUCCESS_CODE))
              (progn
                (print "'js-beautify' is not installed. Trying to install it using npm....")
                (if (eq (shell-command "npm -g install js-beautify") SUCCESS_CODE)
                    (print "'js-beautify' succesfully installed.")
                  (print "Error: `js-beautify' could not be installed.")))))
        (verify-js-beautify-installation)
      ...
      ;; workaround when in Mac OS to show dired mode sorted with dirs first
      (require 'ls-lisp)
